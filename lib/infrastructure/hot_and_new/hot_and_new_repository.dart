...
class HotAndNewRepository implements IHotAndNewRepo {
  @override
  Future<Either<MainFailure, HotAndNewData>> getHotsndNewMovieData() async {
    try {
      final Response response =
          await Dio(BaseOptions()).get(ApiEndPoints.hotandnewMovie);
      // log(response.data.toString());
      if (response.statusCode == 200 || response.statusCode == 201) {
        final searchResult = HotAndNewData.fromJson(response.data);
  ...
  @override
  Future<Either<MainFailure, HotAndNewData>> getHotsndNewTvData() async{
    try {
      final Response response =
